...
/**
 * Media Portfolios Podcasting Podcast Playlist Widget
 *
 ...
 * @since 		1.15.0
 */
class MPP_Widget_Playlist extends WP_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a playlist of episodes.', 'mps-podcasts' );
		$this->widget_idbase = 'ss_podcast';
		$this->widget_title = __( 'Podcast: Playlist', 'mps-podcasts' );
...
?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title (optional):', 'mps-podcasts' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" placeholder="<?php _e( 'Widget title', 'mps-podcasts' ); ?>" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'episodes' ); ?>"><?php _e( 'Episodes (comma-separated IDs):', 'mps-podcasts' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'episodes' ); ?>" name="<?php echo $this->get_field_name( 'episodes' ); ?>" type="text" placeholder="<?php _e( 'Display all episodes', 'mps-podcasts' ); ?>" value="<?php echo $episodes; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'series_slug' ); ?>"><?php _e( 'Series:', 'mps-podcasts' ); ?></label>
		<select id="<?php echo $this->get_field_id( 'series_slug' ); ?>" name="<?php echo $this->get_field_name( 'series_slug' ); ?>">
			<option value=""><?php _e( 'Use episodes specified above', 'mps-podcasts' ); ?></option>
			<?php
			foreach ( $series as $s ) {
