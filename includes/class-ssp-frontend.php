...
class MPP_Frontend {
	...
	public function __construct( $file, $version ) {
		...
		// Add MPP label and version to generator tags
	...
	public function html5_player_conditional_scripts() {
		...
		?>
		<link rel="stylesheet" href="//fonts.googleapis.com/css?family=Roboto:400,700&v=<?php echo MPP_VERSION ?>" />
		<link rel="stylesheet" href="<?php echo MPP_PLUGIN_URL ?>assets/css/icon_fonts.css?v=<?php echo MPP_VERSION ?>" />
		<link rel="stylesheet" href="<?php echo MPP_PLUGIN_URL ?>assets/fonts/Gizmo/gizmo.css?v=<?php echo MPP_VERSION ?>" />
		<link rel="stylesheet" href="<?php echo MPP_PLUGIN_URL ?>assets/css/frontend.css?v=<?php echo MPP_VERSION ?>" />
		<script src="//cdnjs.cloudflare.com/ajax/libs/wavesurfer.js/1.4.0/wavesurfer.min.js?v=<?php echo MPP_VERSION ?>"></script>
		<?php
	}
	...
	public function load_styles_and_scripts() {
		...
		wp_register_script( 'media-player', MPP_PLUGIN_URL . 'assets/js/media.player.js', array( 'jquery' ), $this->version, true );
	...
	private function get_no_album_art_image_array() {
		$src    = MPP_PLUGIN_URL . '/assets/images/no-album-art.png';
		$width  = 300;
		$height = 300;
										...
										<div class="ssp-media-player">
											<div class="ssp-custom-player-controls">
												...
												<div class="ssp-wave-form">
													<div class="ssp-inner">
														<div id="waveform<?php echo $large_player_instance_number; ?>" class="ssp-wave"></div>
													</div>
												</div>
						...
						<script>
							...
							document.addEventListener("DOMContentLoaded", function () {
								(function ($) {
									...
									// Create Player
									window.ssp_player<?php echo $large_player_instance_number; ?> = WaveSurfer.create({
										container: '#waveform<?php echo $large_player_instance_number; ?>',
										waveColor: '#444',
									...
									//Set player track
									window.ssp_player<?php echo $large_player_instance_number; ?>.track = '<?php echo $srcFile; ?>';

									/**
									 * Setting and drawing the peaks seems to be required for the 'load on play' functionality to work
									 */
									//Set peaks
									window.ssp_player<?php echo $large_player_instance_number; ?>.backend.peaks = [0.0218, 0.0183, 0.0165, 0.0198, 0.2137, 0.2888, 0.2313, 0.15, 0.2542, 0.2538, 0.2358, 0.1195, 0.1591, 0.2599, 0.2742, 0.1447, 0.2328, 0.1878, 0.1988, 0.1645, 0.1218, 0.2005, 0.2828, 0.2051, 0.1664, 0.1181, 0.1621, 0.2966, 0.189, 0.246, 0.2445, 0.1621, 0.1618, 0.189, 0.2354, 0.1561, 0.1638, 0.2799, 0.0923, 0.1659, 0.1675, 0.1268, 0.0984, 0.0997, 0.1248, 0.1495, 0.1431, 0.1236, 0.1755, 0.1183, 0.1349, 0.1018, 0.1109, 0.1833, 0.1813, 0.1422, 0.0961, 0.1191, 0.0791, 0.0631, 0.0315, 0.0157, 0.0166, 0.0108];

									//Draw peaks
									window.ssp_player<?php echo $large_player_instance_number; ?>.drawBuffer();

									//Variable to check if the track is loaded
									window.ssp_player<?php echo $large_player_instance_number; ?>.loaded = false;
	...
	public function episode_meta_details ( $episode_id = 0, $context = 'content', $return = false ) {
		...
		foreach ( $meta as $key => $data ) {
			...
			switch( $key ) {
				...
				case 'link':
					$podcast_display .= '<a href="' . esc_url( $data ) . '" title="' . get_the_title() . ' " class="podcast-meta-download">' . __( 'Download file' , 'mps-podcasts' ) . '</a>';
					break;

				case 'new_window':
					$play_link = add_query_arg( 'ref', 'new_window', $link );
					$podcast_display .= '<a href="' . esc_url( $play_link ) . '" target="_blank" title="' . get_the_title() . ' " class="podcast-meta-new-window">' . __( 'Play in new window' , 'mps-podcasts' ) . '</a>';
					break;

				case 'duration':
					$podcast_display .= '<span class="podcast-meta-duration">' . __( 'Duration' , 'mps-podcasts' ) . ': ' . $data . '</span>';
					break;

				case 'date_recorded':
					$podcast_display .= '<span class="podcast-meta-date">' . __( 'Recorded on' , 'mps-podcasts' ) . ' ' . date_i18n( get_option( 'date_format' ), strtotime( $data ) ) . '</span>';
					break;
		...
		if( $itunes_share_url = get_option( 'ss_podcasting_itunes_url_' . $episode_series ) ){
			...
			//$meta_display .= $meta_sep . '<a href="' . esc_url( $itunes_share_url ) . '" title="' . __( 'View on iTunes', 'mps-podcasts' ) . '" class="podcast-meta-itunes">' . __( 'iTunes', 'mps-podcasts' ) . '</a>';
		}
		...
		if( $google_play_share_url = get_option( 'ss_podcasting_google_play_url_' . $episode_series ) ){
			...
			//$meta_display .= $meta_sep . '<a href="' . esc_url( $google_play_share_url ) . '" title="' . __( 'View on Google Play', 'mps-podcasts' ) . '" class="podcast-meta-itunes">' . __( 'Google Play', 'mps-podcasts' ) . '</a>';
		}

		if( $stitcher_share_url = get_option( 'ss_podcasting_stitcher_url_' . $episode_series ) ){
			$share_url_array['Stitcher'] = $stitcher_share_url;
			//$meta_display .= $meta_sep . '<a href="' . esc_url( $stitcher_share_url ) . '" title="' . __( 'View on Stitcher', 'mps-podcasts' ) . '" class="podcast-meta-itunes">' . __( 'Stitcher', 'mps-podcasts' ) . '</a>';
		}
		...
		foreach( $subscribe_urls as $key => $data ){
			...
			switch( $key ) {
				...
				case 'itunes_url':
					$subscribe_display .= '<a href="' . esc_url( $data ) . '" target="_blank" title="' . apply_filters( 'ssp_subscribe_link_name_itunes', __( 'iTunes', 'mps-podcasts' ) ) . '" class="podcast-meta-itunes">' . apply_filters( 'ssp_subscribe_link_name_itunes', __( 'iTunes', 'mps-podcasts' ) ) . '</a>';
				break;

				case 'stitcher_url':
					$subscribe_display .= '<a href="' . esc_url( $data ) . '" target="_blank" title="' . apply_filters( 'ssp_subscribe_link_name_stitcher', __( 'Stitcher', 'mps-podcasts' ) ) . '" class="podcast-meta-itunes">' . apply_filters( 'ssp_subscribe_link_name_stitcher', __( 'Stitcher', 'mps-podcasts' ) ) . '</a>';
				break;

				case 'google_play_url':
					$subscribe_display .= '<a href="' . esc_url( $data ) . '" target="_blank" title="' . apply_filters( 'ssp_subscribe_link_name_google_play', __( 'Google Play', 'mps-podcasts' ) ) . '" class="podcast-meta-itunes">' . apply_filters( 'ssp_subscribe_link_name_google_play', __( 'Google Play', 'mps-podcasts' ) ) . '</a>';
				break;

				case 'spotify_url':
					$subscribe_display .= '<a href="' . esc_url( $data ) . '" target="_blank" title="' . apply_filters( 'ssp_subscribe_link_name_spotify', __( 'Spotify', 'mps-podcasts' ) ) . '" class="podcast-meta-itunes">' . apply_filters( 'ssp_subscribe_link_name_spotify', __( 'Spotify', 'mps-podcasts' ) ) . '</a>';
					break;
		...
		if ( ! empty( $podcast_display ) || ! empty( $subscribe_display ) ) {
			...
			if ( ! empty( $subscribe_display ) ) {
				$subscribe_display = '<p>' . __( 'Subscribe:', 'mps-podcasts' ) . ' ' . $subscribe_display . '</p>';
	...
	public function generator_tag( $gen, $type ) {
		...
		if ( apply_filters( 'ssp_show_generator_tag', true, $type ) ) {
			...
			$generator = 'Media Portfolios Podcasting ' . esc_attr( $this->version );
	...
	public function rss_meta_tag() {
		...
		if( apply_filters( 'ssp_show_global_feed_tag', true ) ) {
			$html = '<link rel="alternate" type="application/rss+xml" title="' . __( 'Podcast RSS feed', 'mps-podcasts' ) . '" href="' . esc_url( $feed_url ) . '" />';
		}
		...
		if( isset( $current_obj->taxonomy ) && 'series' == $current_obj->taxonomy && isset( $current_obj->slug ) && $current_obj->slug ) {
			...
			if( apply_filters( 'ssp_show_series_feed_tag', true, $current_obj->slug ) ) {
				...
				$html .= "\n" . '<link rel="alternate" type="application/rss+xml" title="' . sprintf( __( '%s RSS feed', 'mps-podcasts' ), $current_obj->name ) . '" href="' . esc_url( $series_feed_url ) . '" />';
	...
	public function register_widgets () {
		...
		foreach ( $widgets as $id => $name ) {
			...
			register_widget( 'MPP_Widget_' . $name );
		}
	...
	public function podcast_episode ( $episode_id = 0, $content_items = array( 'title', 'player', 'details' ), $context = '', $style = 'mini' ) {
		...
		$html = '<div class="podcast-episode episode-' . esc_attr( $episode_id ) . '">' . "\n";
			...
			if ( ! empty( $thumb_id ) ) {
				...
				$albumArt['src'] = MPP_PLUGIN_URL . '/assets/images/no-album-art.png';
				$albumArt['width'] = 300;
				$albumArt['height'] = 300;
			}
			 ...
			 * @author Simon Dowdles - MPP <simon.dowdles@gmail.com>
	...
	public function load_localisation () {
		load_plugin_textdomain( 'mps-podcasts', false, basename( dirname( $this->file ) ) . '/languages/' );
	}
